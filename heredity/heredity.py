...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for child in people.keys():
    	if child not in one_gene and child not in two_genes:
    		zero_gene.append(child)
    	if child not in have_trait:
    		not_have_trait.append(child)

    for child in zero_gene:
    	prob=1
    	...
    	if people[child]["father"]==None:
    		...
    		#given both are in 0 gene only by not mutation they cannot pass the trait
    		if mother in zero_gene and father in zero_gene:
    			prob*=(1-PROBS["mutation"])*(1-PROBS["mutation"])
    		elif (mother in zero_gene and father in one_gene) or (father in zero_gene and mother in one_gene):
    			prob*=(1-PROBS["mutation"])*0.5
    		elif (mother in zero_gene and father in two_genes) or (father in zero_gene and mother in two_genes):
    			prob*=(1-PROBS["mutation"])*PROBS["mutation"]
    		elif mother in one_gene and father in one_gene:
    			prob*=0.5*0.5
    		elif (mother in one_gene and father in two_genes) or (father in one_gene and mother in two_genes):
    ...
    for child in one_gene:
    	...
    	if people[child]["father"]==None:
    		...
    		#given both are in 0 gene only by mutation they can pass the trait
    		if mother in zero_gene and father in zero_gene:
    			prob*=(1-PROBS["mutation"])*PROBS["mutation"]+(1-PROBS["mutation"])*PROBS["mutation"]
    		elif (mother in zero_gene and father in one_gene) or (father in zero_gene and mother in one_gene):
    			prob*=(1-PROBS["mutation"])*0.5+PROBS["mutation"]*0.5
    		#he gets the gene from his mother and not his father, or he gets the gene from his father and not his mother.
    		elif (mother in zero_gene and father in two_genes) or (father in zero_gene and mother in two_genes):
    			prob*=PROBS["mutation"]*PROBS["mutation"]+(1-PROBS["mutation"])*(1-PROBS["mutation"])
    		elif mother in one_gene and father in one_gene:
    			prob*=0.5*0.5+0.5*0.5
    		elif (mother in one_gene and father in two_genes) or (father in one_gene and mother in two_genes):
    			prob*=0.5*PROBS["mutation"]+(1-PROBS["mutation"])*0.5
    		else:
...
def normalize(probabilities):
    ...
    """
    for person in probabilities.keys():
    	temp=sum(probabilities[person]["trait"].values())
    	probabilities[person]["trait"][True]/=temp
    	probabilities[person]["trait"][False]/=temp

    	temp1=sum(probabilities[person]["gene"].values())
    	probabilities[person]["gene"][0]/=temp1
    	probabilities[person]["gene"][1]/=temp1
    	probabilities[person]["gene"][2]/=temp1
